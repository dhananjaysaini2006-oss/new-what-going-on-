...
Official authorities and reporting correspondents on the ground continue to monitor developments closely as further communiqués and statements are anticipated in the coming hours.`}resolveContextualNewsImage(e,t,s,a,i=0){if(e&&!e.includes("1x1")&&!e.includes("pixel")&&!e.includes("favicon"))return e;const n=`${t} ${s}`.toLowerCase();return a.region==="india"||a.isPolitics||n.includes("india")||n.includes("delhi")?n.includes("supreme court")||n.includes("high court")||n.includes("judge")||n.includes("judiciary")||n.includes("verdict")||n.includes("bench")||n.includes("bail")||n.includes("cbi")||n.includes("ed")?w.judiciary_law[0]:n.includes("isro")||n.includes("space")||n.includes("satellite")||n.includes("chandrayaan")||n.includes("drdo")||n.includes("missile")?w.defense_isro[i%w.defense_isro.length]:n.includes("rupee")||n.includes("rbi")||n.includes("reserve bank")||n.includes("bank")||n.includes("sensex")||n.includes("nifty")||n.includes("gdp")||n.includes("inflation")||n.includes("finance minister")?w.economy_rbi[i%w.economy_rbi.length]:n.includes("solar")||n.includes("renewable")||n.includes("power")||n.includes("green energy")?w.clean_energy[0]:n.includes("semiconductor")||n.includes("chip")||n.includes("fab")||n.includes("hardware")?w.technology[0]:w.parliament_governance[i%w.parliament_governance.length]:$[i%$.length]}isPoliticalArticle(e,t){const s=`${e} ${t}`.toLowerCase();return["parliament","lok sabha","rajya sabha","bjp","congress","election","elections","minister","cabinet","supreme court","chief minister","prime minister","modi","rahul gandhi","governor","mla","mp","assembly","ordinance","bill","judiciary","constitution","poll","party","yogi","kejriwal","shah","niti aayog","electoral","high court"].some(i=>s.includes(i))}parseDateSafe(e){if(!e)return new Date().toISOString();const t=new Date(e);return isNaN(t.getTime())?new Date().toISOString():t.toISOString()}stripHtml(e){if(!e)return"";const t=document.createElement("DIV");return t.innerHTML=e,t.textContent||t.innerText||""}extractCleanSnippet(e){const t=this.stripHtml(e);if(!t)return"Read the complete verified story on the original publisher broadcast.";const s=t.replace(/\s+/g," ").trim();return s.length>260?s.substring(0,257)+"...":s}extractImageFromXml(e,t){const s=Array.from(e.querySelectorAll("enclosure"));for(const n of s){const o=n.getAttribute("type")||"",l=n.getAttribute("url");if(l&&(o.startsWith("image")||l.match(/\.(jpeg|jpg|png|webp)/i)))return l}const a=Array.from(e.getElementsByTagNameNS("*","content")).concat(Array.from(e.getElementsByTagNameNS("*","thumbnail")));for(const n of a){const o=n.getAttribute("url");if(o&&!o.includes("1x1")&&!o.includes("analytics")&&!o.includes("logo"))return o}const i=e.querySelector("storyimage, thumb, image > url, fullimage");if(i&&i.textContent){const n=i.textContent.trim();if(n.startsWith("http"))return n}return this.extractImageFromHtml(t)}extractImageFromHtml(e){if(e){const t=e.matchAll(/<img[^>]+(?:src|data-src|data-original)=["']([^"']+)["']/gi);for(const s of t){const a=s[1];if(a&&!a.includes("feedburner")&&!a.includes("analytics")&&!a.includes("1x1")&&!a.includes("pixel")&&!a.includes("badge")&&!a.includes("button")&&!a.includes("icon"))return a}}return null}generateKeyPoints(e,t,s){return[`Primary Development: ${e}`,`Core Briefing: ${t.length>30?t.substring(0,140)+"...":"Verified wire bulletin from "+s.name+"."}`,`Verified Media Wire: Real-time update monitored on ${s.name} editorial network.`]}}const j=new F;class V{constructor(){this.synth=window.speechSynthesis,this.currentUtterance=null,this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.callbacks=[],this.voices=[],this.synth&&(this.loadVoices(),speechSynthesis.onvoiceschanged!==void 0&&(speechSynthesis.onvoiceschanged=()=>this.loadVoices()))}loadVoices(){this.voices=this.synth.getVoices()}getBestVoice(){return this.voices.length||this.loadVoices(),this.voices.find(t=>t.lang.startsWith("en")&&(t.name.includes("Natural")||t.name.includes("Google")||t.name.includes("Premium")))||this.voices.find(t=>t.lang.startsWith("en"))||this.voices[0]}speakArticle(e,t){if(!this.synth){console.warn("Speech synthesis not supported in this browser.");return}if(this.isPlaying&&this.currentArticleId===e.id){this.isPaused?this.resume():this.pause();return}this.stop();const s=e.title?e.title.replace(/<[^>]*>?/gm,""):"",a=e.snippet?e.snippet.replace(/<[^>]*>?/gm,""):"",i=e.content?e.content.replace(/<[^>]*>?/gm,""):"",n=`Story from ${e.source}. Headline: ${s}. Summary: ${a||i}`;this.currentUtterance=new SpeechSynthesisUtterance(n),this.currentUtterance.voice=this.getBestVoice(),this.currentUtterance.rate=1,this.currentUtterance.pitch=1,this.currentArticleId=e.id,this.isPlaying=!0,this.isPaused=!1,this.currentUtterance.onstart=()=>{this.notifyStateChange("playing",e)},this.currentUtterance.onend=()=>{this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.notifyStateChange("ended",e),t&&t()},this.currentUtterance.onerror=o=>{console.warn("Speech synthesis error",o),this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.notifyStateChange("ended",e)},this.synth.speak(this.currentUtterance)}pause(){this.synth&&this.isPlaying&&!this.isPaused&&(this.synth.pause(),this.isPaused=!0,this.notifyStateChange("paused"))}resume(){this.synth&&this.isPlaying&&this.isPaused&&(this.synth.resume(),this.isPaused=!1,this.notifyStateChange("playing"))}stop(){this.synth&&(this.synth.cancel(),this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.notifyStateChange("stopped"))}subscribe(e){return this.callbacks.push(e),()=>{this.callbacks=this.callbacks.filter(t=>t!==e)}}notifyStateChange(e,t=null){this.callbacks.forEach(s=>s({state:e,article:t,articleId:this.currentArticleId,isPlaying:this.isPlaying,isPaused:this.isPaused}))}}const T=new V,K="modulepreload",J=function(g){return"/"+g},D={},C=function(e,t,s){let a=Promise.resolve();if(t&&t.length>0){let n=function(h){return Promise.all(h.map(c=>Promise.resolve(c).then(r=>({status:"fulfilled",value:r}),r=>({status:"rejected",reason:r}))))};document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),l=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));a=n(t.map(h=>{if(h=J(h),h in D)return;D[h]=!0;const c=h.endsWith(".css"),r=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${h}"]${r}`))return;const u=document.createElement("link");if(u.rel=c?"stylesheet":K,c||(u.as="script"),u.crossOrigin="",u.href=h,l&&u.setAttribute("nonce",l),document.head.appendChild(u),c)return new Promise((p,v)=>{u.addEventListener("load",p),u.addEventListener("error",()=>v(new Error(`Unable to preload CSS for ${h}`)))})}))}function i(n){const o=new Event("vite:preloadError",{cancelable:!0});if(o.payload=n,window.dispatchEvent(o),!o.defaultPrevented)throw n}return a.then(n=>{for(const o of n||[])o.status==="rejected"&&i(o.reason);return e().catch(i)})};class Y{constructor(e,t,s,a,i,n,o){this.onRefreshCallback=e,this.onSearchCallback=t,this.onThemeToggleCallback=s,this.onIntervalChangeCallback=a,this.onPublishClickCallback=i,this.onAuthClickCallback=n,this.onAdminClickCallback=o,this.remainingSeconds=m.getRefreshInterval(),this.totalSeconds=this.remainingSeconds,this.countdownTimer=null,this.clockTimer=null}init(){this.renderHeader(),this.startLiveClock(),this.startCountdown(),this.attachEventListeners(),this.updateBookmarkBadge(),this.initMobileDrawer()}renderHeader(){const e=document.getElementById("top-utility-bar"),t=document.getElementById("main-header");e&&(e.innerHTML=`
      ...
      `+l.map(c=>`
        <button
          ...
          data-cat="${c.id}"
        >${c.label}</button>
      `).join(""),(h=i.querySelector("#btn-mobile-drawer-publish-cta"))==null||h.addEventListener("click",()=>{o(),this.onPublishClickCallback&&this.onPublishClickCallback()}),i.querySelectorAll(".mobile-drawer-cat-btn").forEach(c=>{c.addEventListener("click",()=>{const r=document.querySelector(`.cat-tab[data-category="${c.dataset.cat}"]`);r&&r.click(),i.querySelectorAll(".mobile-drawer-cat-btn").forEach(u=>u.classList.remove("active")),c.classList.add("active"),o(),window.scrollTo({top:0,behavior:"smooth"})})}))}startLiveClock(){const e=()=>{const t=new Date,s=document.getElementById("current-clock-time"),a=document.getElementById("current-clock-date");s&&(s.textContent=t.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0})),a&&(a.textContent=t.toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric",year:"numeric"}))};e(),this.clockTimer=setInterval(e,1e3)}startCountdown(){this.countdownTimer&&clearInterval(this.countdownTimer),this.totalSeconds=m.getRefreshInterval(),this.remainingSeconds=this.totalSeconds,this.nextRefreshTime=Date.now()+this.totalSeconds*1e3;const e=document.getElementById("countdown-circle-bar"),t=document.getElementById("countdown-text"),s=2*Math.PI*7;e&&(e.style.strokeDasharray=`${s}`);const a=()=>{const i=Date.now(),n=this.nextRefreshTime-i,o=Math.max(0,Math.ceil(n/1e3));this.remainingSeconds=o;const l=Math.floor(this.remainingSeconds/60),h=this.remainingSeconds%60;if(t&&(t.textContent=`${l.toString().padStart(2,"0")}:${h.toString().padStart(2,"0")}`),e){const c=Math.min(1,Math.max(0,this.remainingSeconds/this.totalSeconds)),r=s*(1-c);e.style.strokeDashoffset=`${r}`}o<=0&&(this.resetCountdown(),t&&(t.textContent="Syncing..."),this.onRefreshCallback&&this.onRefreshCallback(!0))};a(),this.countdownTimer=setInterval(a,1e3),this.visibilityListenerAttached||(this.visibilityListenerAttached=!0,document.addEventListener("visibilitychange",()=>{document.visibilityState==="visible"&&a()}))}resetCountdown(){this.totalSeconds=m.getRefreshInterval(),this.remainingSeconds=this.totalSeconds,this.nextRefreshTime=Date.now()+this.totalSeconds*1e3}updateLastSyncTime(e=new Date){const t=document.getElementById("last-sync-time");if(t){const s=e.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0});t.textContent=`Synced: ${s}`}}updateBookmarkBadge(){const e=document.getElementById("header-bookmark-badge"),t=m.getBookmarks().length;e&&(e.textContent=t,e.style.display=t>0?"flex":"none")}updateUserAuthState(e){var n,o,l,h,c;const t=document.getElementById("header-auth-container"),s=document.getElementById("topbar-auth-pill"),a=document.getElementById("btn-open-publish-modal");if(a&&(a.style.display="inline-flex"),e){if(s&&(s.innerHTML=`
          <span style="color: var(--text-primary); font-weight: 800; font-size: 0.775rem; display: inline-flex; align-items: center; gap: 4px;">
            ${isAdmin?"👑 Dhananjay":`👤 ${(e.displayName||e.email).split(" ")[0]}`}
          </span>
        `),t){t.innerHTML=`
          ${isAdmin?`
        ...
        `;const r=t.querySelector("#btn-header-admin-panel");r&&r.addEventListener("click",()=>{this.onAdminClickCallback&&this.onAdminClickCallback()});const u=t.querySelector("#btn-header-signout");u&&u.addEventListener("click",async()=>{const{firebaseService:p}=await C(async()=>{const{firebaseService:v}=await Promise.resolve().then(()=>U);return{firebaseService:v}},void 0);await p.signOut()})}}else{if(s){s.innerHTML=`
        ...
        `;const r=s.querySelector("#btn-topbar-signin");r&&r.addEventListener("click",p=>{p.preventDefault(),this.onAuthClickCallback&&this.onAuthClickCallback("signin")});const u=s.querySelector("#btn-topbar-signup");u&&u.addEventListener("click",p=>{p.preventDefault(),this.onAuthClickCallback&&this.onAuthClickCallback("signup")})}if(t){t.innerHTML=`
        ...
        `;const r=t.querySelector("#btn-header-signin");r&&r.addEventListener("click",()=>{this.onAuthClickCallback&&this.onAuthClickCallback("signin")});const u=t.querySelector("#btn-header-signup");u&&u.addEventListener("click",()=>{this.onAuthClickCallback&&this.onAuthClickCallback("signup")})}}const i=document.getElementById("mobile-drawer-auth");i&&(e?(i.innerHTML=`
        ...
        `,isAdmin&&((l=i.querySelector("#btn-drawer-write"))==null||l.addEventListener("click",()=>{var u;(u=document.getElementById("mobile-nav-drawer"))==null||u.classList.remove("open");const r=document.getElementById("mobile-drawer-overlay");r&&(r.style.display="none"),document.body.style.overflow="",this.onPublishClickCallback&&this.onPublishClickCallback()}),(h=i.querySelector("#btn-drawer-admin"))==null||h.addEventListener("click",()=>{var u;(u=document.getElementById("mobile-nav-drawer"))==null||u.classList.remove("open");const r=document.getElementById("mobile-drawer-overlay");r&&(r.style.display="none"),document.body.style.overflow="",this.onAdminClickCallback&&this.onAdminClickCallback()})),(c=i.querySelector("#btn-drawer-signout"))==null||c.addEventListener("click",async()=>{var p;(p=document.getElementById("mobile-nav-drawer"))==null||p.classList.remove("open");const r=document.getElementById("mobile-drawer-overlay");r&&(r.style.display="none"),document.body.style.overflow="";const{firebaseService:u}=await C(async()=>{const{firebaseService:v}=await Promise.resolve().then(()=>U);return{firebaseService:v}},void 0);await u.signOut()})):(i.innerHTML=`
        ...
        `,(n=i.querySelector("#btn-drawer-signin"))==null||n.addEventListener("click",()=>{var u;(u=document.getElementById("mobile-nav-drawer"))==null||u.classList.remove("open");const r=document.getElementById("mobile-drawer-overlay");r&&(r.style.display="none"),document.body.style.overflow="",this.onAuthClickCallback&&this.onAuthClickCallback("signin")}),(o=i.querySelector("#btn-drawer-signup"))==null||o.addEventListener("click",()=>{var u;(u=document.getElementById("mobile-nav-drawer"))==null||u.classList.remove("open");const r=document.getElementById("mobile-drawer-overlay");r&&(r.style.display="none"),document.body.style.overflow="",this.onAuthClickCallback&&this.onAuthClickCallback("signup")})))}attachEventListeners(){const e=document.getElementById("btn-header-refresh");e&&e.addEventListener("click",()=>{e.classList.add("spinning"),this.resetCountdown(),this.onRefreshCallback&&this.onRefreshCallback(!1).finally(()=>{setTimeout(()=>e.classList.remove("spinning"),600)})});const t=document.getElementById("select-refresh-interval");t&&(t.value=m.getRefreshInterval().toString(),t.addEventListener("change",c=>{const r=parseInt(c.target.value,10);m.setRefreshInterval(r),this.startCountdown(),this.onIntervalChangeCallback&&this.onIntervalChangeCallback(r)}));const s=document.getElementById("global-search-input"),a=document.getElementById("mobile-search-input"),i=document.getElementById("mobile-search-clear");if(s){let c=null;s.addEventListener("input",r=>{const u=r.target.value;a&&a.value!==u&&(a.value=u,i&&(i.style.display=u?"inline-flex":"none")),clearTimeout(c),c=setTimeout(()=>{this.onSearchCallback&&this.onSearchCallback(u.trim())},250)}),window.addEventListener("keydown",r=>{r.key==="/"&&document.activeElement!==s&&document.activeElement!==a&&(r.preventDefault(),window.innerWidth<=900&&a?a.focus():s.focus())})}if(a){let c=null;a.addEventListener("input",r=>{const u=r.target.value;i&&(i.style.display=u.length>0?"inline-flex":"none"),s&&s.value!==u&&(s.value=u),clearTimeout(c),c=setTimeout(()=>{this.onSearchCallback&&this.onSearchCallback(u.trim())},250)}),i&&i.addEventListener("click",()=>{a.value="",i.style.display="none",s&&(s.value=""),this.onSearchCallback&&this.onSearchCallback("")})}const n=document.getElementById("btn-brand-home");n&&n.addEventListener("click",()=>{s&&(s.value=""),a&&(a.value=""),i&&(i.style.display="none"),this.onSearchCallback&&this.onSearchCallback("");const c=document.querySelector('.cat-tab[data-category="all"]');c&&c.click()});const o=document.getElementById("btn-open-publish-modal");o&&o.addEventListener("click",()=>{this.onPublishClickCallback&&this.onPublishClickCallback()});const l=document.getElementById("topbar-btn-publish");l&&l.addEventListener("click",c=>{c.preventDefault(),this.onPublishClickCallback&&this.onPublishClickCallback()});const h=document.getElementById("btn-theme-toggle");h&&h.addEventListener("click",()=>{this.onThemeToggleCallback&&this.onThemeToggleCallback()})}}class Q{constructor(e){this.onArticleClickCallback=e,this.articles=[]}render(e){this.articles=e||[];const t=document.getElementById("breaking-news-ticker-container");if(!t)return;if(this.articles.length===0){t.style.display="none";return}t.style.display="block";const s=this.articles.slice(0,10),i=[...s,...s].map((o,l)=>`
    ...
    `,t.querySelectorAll(".ticker-item").forEach(o=>{o.addEventListener("click",()=>{const l=o.getAttribute("data-id"),h=this.articles.find(c=>c.id===l)||this.articles[0];this.onArticleClickCallback&&h&&this.onArticleClickCallback(h)})})}}class X{constructor(e,t){this.onArticleClickCallback=e,this.onBookmarkToggleCallback=t,this.viewMode=m.getViewMode()}setViewMode(e){this.viewMode=e,m.setViewMode(e)}formatTimeAgo(e){try{const t=new Date(e),a=new Date-t,i=Math.floor(a/(1e3*60));if(i<1)return"Just now";if(i<60)return`${i}m ago`;const n=Math.floor(i/60);return n<24?`${n}h ago`:`${Math.floor(n/24)}d ago`}catch{return"Recently"}}render(e,t={}){const s=document.getElementById("main-news-feed");if(!s)return;const{category:a="all",searchQuery:i="",sourceFilter:n="all"}=t;if(!e||e.length===0){const v=t.category==="saved";if(t.category===d.PUBLISHED){s.innerHTML=`
        ...
        `;return}s.innerHTML=v?`<div class="empty-state-premium">
          ...
          </div>`;return}if(a===d.ALL&&!i&&n==="all"){this.renderHomeAllNews(e,s);return}const o=!i&&e.length>0,l=o?e[0]:null,h=o?e.slice(1):e,c=l?this.buildHeroCardHtml(l):"",r=this.buildArticlesGridHtml(h),u=a===d.BUSINESS&&!i?this.buildMarketIndicesDashboardHtml():"";let p="Latest Stories";i?p=`Search Results: "${i}"`:a===d.INDIA_POLITICS?p="🏛️ Indian Politics & National Governance":a===d.INDIA?p="🇮🇳 Indian Media Wires":a===d.WORLD?p="🌍 Global News Wires & International":a===d.BUSINESS?p="📊 Global & Indian Business Wire":a===d.GOOGLE_NEWS?p="🌐 Google News Live Wire":a===d.SAVED?p="🔖 Saved Bookmarks":a===d.PUBLISHED?p="✍️ Community & Published Articles":p=`${a.toUpperCase()} Wire`,s.innerHTML=`
      ${u}
    ...
    `}renderHomeAllNews(e,t){const s=e[0],a=e.slice(1,5),i=e.filter(r=>r.region==="india"||r.category===d.INDIA||r.category===d.INDIA_POLITICS),n=e.filter(r=>r.region==="global"||r.category===d.WORLD||r.category===d.GOOGLE_NEWS),o=e.filter(r=>r.category===d.BUSINESS),l=e.filter(r=>r.category===d.TECH||r.category===d.SCIENCE),h=e.slice(5,13),c=s.region==="india"?"NEW DELHI —":s.region==="global"?"GLOBAL WIRE —":"DISPATCH —";t.innerHTML=`
      ...
      <section class="hindu-top-fold animate-fade-in" id="hindu-top">
          ...
          ${s.keyPoints&&s.keyPoints.length>0?`
            <div class="modal-key-points" style="margin: 4px 0 12px; padding: 14px 18px;">
              ...
              <ul style="margin: 0; padding-left: 18px; font-size: 0.9rem; color: var(--text-secondary); line-height: 1.55;">
                ${s.keyPoints.slice(0,2).map(r=>`<li>${r}</li>`).join("")}
              </ul>
            </div>
          `:""}
        ...
        <div class="hindu-wire-col">
          ...
          ${a.map(r=>`
            <article class="hindu-wire-item" data-id="${r.id}">
              <div style="display: flex; justify-content: space-between; align-items: center;">
                <span class="hindu-wire-tag">${(r.category||"GENERAL").toUpperCase()}</span>
                <span style="font-size: 0.725rem; color: var(--text-muted); font-family: var(--font-mono);">${this.formatTimeAgo(r.pubDate)}</span>
              </div>
              <h4 class="hindu-wire-headline" data-action="open-article">${r.title}</h4>
              <div class="hindu-wire-meta">
                <span style="font-weight: 700; color: var(--accent-cyan);">${r.source}</span>
                <span style="color: var(--text-muted);">•</span>
                <span style="color: var(--text-muted);">${r.readTime||"2 min read"}</span>
              </div>
            </article>
          `).join("")}
        </div>
    ...
    `}buildArticlesGridHtml(e){return this.viewMode==="grid"?`
      ...
      `}attachCardEventListeners(e){const t=document.getElementById("main-news-feed");t&&t.addEventListener("click",s=>{const a=s.target.closest("[data-action]"),i=s.target.closest("[data-id]");if(!i)return;const n=i.getAttribute("data-id"),o=e.find(l=>l.id===n);if(o)if(a){const l=a.getAttribute("data-action");if(l==="open-article")this.onArticleClickCallback&&this.onArticleClickCallback(o);else if(l==="bookmark"){const h=m.toggleBookmark(o);a.classList.toggle("bookmarked",h);const c=a.querySelector("svg");c&&c.setAttribute("fill",h?"currentColor":"none"),this.onBookmarkToggleCallback&&this.onBookmarkToggleCallback(o,h)}else l==="speak-article"?T.speakArticle(o):l==="share"&&(navigator.share?navigator.share({title:o.title,text:o.snippet,url:o.link||window.location.href}).catch(()=>{}):(navigator.clipboard.writeText(o.link||window.location.href),alert("Article link copied to clipboard!")))}else s.target.closest("button")||this.onArticleClickCallback&&this.onArticleClickCallback(o)})}}class Z{constructor(){this.activeTopic="all",this.currentAffairsList=[...W]}render(){const e=document.getElementById("main-news-feed");if(!e)return;const t=this.activeTopic==="all"?this.currentAffairsList:this.currentAffairsList.filter(i=>i.category===this.activeTopic),a=new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"});e.innerHTML=`
...
`).filter(l=>l.trim().length>0);t.innerHTML=`
    ...
    `,t.classList.add("active"),document.body.style.overflow="hidden",this.attachModalEvents(e);const n=t.querySelector(".modal-scroll-content"),o=document.getElementById("modal-read-progress-bar");n&&o&&n.addEventListener("scroll",()=>{const l=n.scrollTop,h=n.scrollHeight-n.clientHeight,c=h>0?Math.min(100,l/h*100):0;o.style.width=c+"%"},{passive:!0})}close(){const e=document.getElementById("article-reader-modal");e&&(e.classList.remove("active"),document.body.style.overflow="")}attachModalEvents(e){const t=document.getElementById("modal-close-btn");t&&t.addEventListener("click",()=>this.close());const s=document.getElementById("modal-article-body-text"),a=document.getElementById("modal-font-inc"),i=document.getElementById("modal-font-dec");a&&s&&a.addEventListener("click",()=>{this.fontSize<24&&(this.fontSize+=1.5,s.style.fontSize=`${this.fontSize}px`)}),i&&s&&i.addEventListener("click",()=>{this.fontSize>14&&(this.fontSize-=1.5,s.style.fontSize=`${this.fontSize}px`)});const n=document.getElementById("modal-audio-btn");n&&n.addEventListener("click",()=>{T.speakArticle(e)});const o=document.getElementById("modal-bookmark-btn");o&&o.addEventListener("click",()=>{const r=m.toggleBookmark(e);o.classList.toggle("bookmarked",r);const u=o.querySelector("svg");u&&u.setAttribute("fill",r?"currentColor":"none"),this.onBookmarkToggleCallback&&this.onBookmarkToggleCallback(e,r)});const l=document.getElementById("modal-copy-link-btn");l&&l.addEventListener("click",()=>{navigator.clipboard.writeText(e.link||window.location.href),l.textContent="Copied!",setTimeout(()=>{l.textContent="Copy Link"},2e3)});const h=document.getElementById("modal-whatsapp-btn");h&&h.addEventListener("click",async()=>{const r={title:e.title,text:e.snippet||e.title,url:e.link||window.location.href};try{if(navigator.share)await navigator.share(r);else{await navigator.clipboard.writeText(e.link||window.location.href);const u=h.innerHTML;h.textContent="Link Copied!",setTimeout(()=>{h.innerHTML=u},2e3)}}catch{}});const c=document.getElementById("modal-delete-user-article-btn");c&&c.addEventListener("click",()=>{confirm("Are you sure you want to unpublish this story from What's Going On?")&&(m.deleteCustomArticle(e.id),this.close(),window.location.reload())})}attachGlobalListeners(){window.addEventListener("keydown",t=>{t.key==="Escape"&&this.close()});const e=document.getElementById("article-reader-modal");e&&e.addEventListener("click",t=>{t.target===e&&this.close()})}}const I={apiKey:"AIzaSyDemoPlaceholderKeyForWhatsGoingOn",authDomain:"whats-going-on-news.firebaseapp.com",projectId:"whats-going-on-news",storageBucket:"whats-going-on-news.appspot.com",messagingSenderId:"123456789012",appId:"1:123456789012:web:abcdef123456"},y={CURRENT_USER:"wgo_auth_user_v1",REGISTERED_USERS:"wgo_registered_users_v1",BREAKING_ALERT:"wgo_breaking_alert_v1"},P=1440*60*1e3;function B(g){try{return btoa(encodeURIComponent(g+"_wgo_salt_2026"))}catch{return g}}function R(g,e){return B(g)===e}const k={e:"dhananjaysaini2006@gmail.com",h:B("242622")};class te{constructor(){this.currentUser=null,this.authListeners=[],this.isFirebaseLive=!1,this.firebaseApp=null,this.firebaseAuth=null,this.init()}async init(){this.loadSavedSession(),this.ensureAdminExists();try{if(I.apiKey&&!I.apiKey.includes("Placeholder")){const{initializeApp:e}=await C(async()=>{const{initializeApp:s}=await import("./index.esm-D_m3LDGF.js");return{initializeApp:s}},__vite__mapDeps([0,1])),{getAuth:t}=await C(async()=>{const{getAuth:s}=await import("./index.esm-8z07djW7.js");return{getAuth:s}},__vite__mapDeps([2,1]));this.firebaseApp=e(I),this.firebaseAuth=t(this.firebaseApp),this.isFirebaseLive=!0,console.info("Connected to live Firebase Cloud project.")}}catch{console.info("Using local Firebase Authentication adapter.")}}loadSavedSession(){try{const e=localStorage.getItem(y.CURRENT_USER);e&&(this.currentUser=JSON.parse(e))}catch{this.currentUser=null}}ensureAdminExists(){const e=this.getAllUsers();if(!e.some(s=>s.email.toLowerCase()===k.e.toLowerCase())){e.unshift({uid:"admin-ds-wgo",email:k.e,displayName:"Dhananjay Saini",role:"admin",passwordHash:k.h,createdAt:"2026-09-02T00:00:00.000Z"});try{localStorage.setItem(y.REGISTERED_USERS,JSON.stringify(e))}catch{}}}getAllUsers(){try{const e=localStorage.getItem(y.REGISTERED_USERS);return e?JSON.parse(e):[]}catch{return[]}}getCurrentUser(){return this.currentUser}isAdmin(){return this.currentUser&&(this.currentUser.role==="admin"||this.currentUser.email&&this.currentUser.email.toLowerCase()===k.e.toLowerCase())}onAuthStateChanged(e){return this.authListeners.push(e),e(this.currentUser),()=>{this.authListeners=this.authListeners.filter(t=>t!==e)}}notifyAuthChanged(){this.authListeners.forEach(e=>{try{e(this.currentUser)}catch(t){console.error("Error in auth listener:",t)}})}async signIn(e,t){const s=(e||"").trim().toLowerCase(),a=(t||"").trim();if(!s||!a)throw new Error("Please provide both email and password.");if(!s.includes("@"))throw new Error("Please enter a valid email address.");if(s===k.e.toLowerCase()){if(R(a,k.h))return this.currentUser={uid:"admin-ds-wgo",email:k.e,displayName:"Dhananjay Saini",role:"admin",photoURL:null},localStorage.setItem(y.CURRENT_USER,JSON.stringify(this.currentUser)),this.notifyAuthChanged(),this.currentUser;throw new Error("Incorrect password. Please try again.")}const n=this.getAllUsers().find(l=>l.email.toLowerCase()===s);if(!n)throw new Error("No account found with this email. Please sign up first.");if(!(n.passwordHash?R(a,n.passwordHash):n.passwordHash===a))throw new Error("Invalid password. Please try again.");return this.currentUser={uid:n.uid,email:n.email,displayName:n.displayName,role:n.role||"reader",photoURL:n.photoURL||null},localStorage.setItem(y.CURRENT_USER,JSON.stringify(this.currentUser)),this.notifyAuthChanged(),this.currentUser}async signUp(e,t,s=""){const a=(e||"").trim().toLowerCase(),i=(t||"").trim(),n=(s||"").trim()||a.split("@")[0];if(!a||!i)throw new Error("Please provide email and password.");if(!a.includes("@"))throw new Error("Please enter a valid email address.");if(i.length<8)throw new Error("Password must be at least 8 characters long.");const o=this.getAllUsers();if(o.some(h=>h.email.toLowerCase()===a))throw new Error("An account with this email already exists. Please sign in.");const l={uid:"user-"+Date.now(),email:a,displayName:n,role:"reader",passwordHash:B(i),createdAt:new Date().toISOString()};o.push(l);try{localStorage.setItem(y.REGISTERED_USERS,JSON.stringify(o))}catch(h){console.warn("Failed to save user",h)}return this.currentUser={uid:l.uid,email:l.email,displayName:l.displayName,role:l.role,photoURL:null},localStorage.setItem(y.CURRENT_USER,JSON.stringify(this.currentUser)),this.notifyAuthChanged(),this.currentUser}async signOut(){this.currentUser=null,localStorage.removeItem(y.CURRENT_USER),this.notifyAuthChanged()}getBreakingAlert(){try{const e=localStorage.getItem(y.BREAKING_ALERT);if(!e)return null;const t=JSON.parse(e);return t.timestamp&&Date.now()-new Date(t.timestamp).getTime()>P?(localStorage.removeItem(y.BREAKING_ALERT),null):t}catch{return null}}setBreakingAlert(e,t="high"){if(!this.isAdmin())throw new Error("Unauthorized. Only Admins can broadcast alerts.");const s={id:"alert-"+Date.now(),text:e,urgency:t,timestamp:new Date().toISOString(),author:this.currentUser.displayName,expiresAt:new Date(Date.now()+P).toISOString()};return localStorage.setItem(y.BREAKING_ALERT,JSON.stringify(s)),s}clearBreakingAlert(){if(!this.isAdmin())throw new Error("Unauthorized.");localStorage.removeItem(y.BREAKING_ALERT)}}const f=new te,U=Object.freeze(Object.defineProperty({__proto__:null,FIREBASE_CONFIG:I,firebaseService:f},Symbol.toStringTag,{value:"Module"}));class ie{constructor(e){this.onArticlePublishedCallback=e,this.modalEl=null,this.isOpen=!1,this.editingArticleId=null,this.presetImages=[{label:"🏛️ Parliament & Central Vista",url:"https://images.unsplash.com/photo-1587474260584-136574528ed5?w=1200&auto=format&fit=crop&q=80"},{label:"⚖️ Supreme Court & Justice",url:"https://images.unsplash.com/photo-1589829545856-d10d557cf95f?w=1200&auto=format&fit=crop&q=80"},{label:"🪙 Indian Rupee & RBI",url:"https://images.unsplash.com/photo-1607604276583-eef5d076aa5f?w=1200&auto=format&fit=crop&q=80"},{label:"🚀 ISRO Space Rocket",url:"https://images.unsplash.com/photo-1517976487508-59a5a0da23b2?w=1200&auto=format&fit=crop&q=80"},{label:"🤖 Semiconductors & AI",url:"https://images.unsplash.com/photo-1518770660439-4636190af475?w=1200&auto=format&fit=crop&q=80"},{label:"🌍 Global Summit & Diplomacy",url:"https://images.unsplash.com/photo-1511578314322-379afb476865?w=1200&auto=format&fit=crop&q=80"}],this.selectedImageUrl=this.presetImages[0].url}init(){this.injectModalHtml(),this.attachEventListeners()}injectModalHtml(){const e=document.getElementById("publish-studio-modal");e&&e.remove();const t=document.createElement("div");t.id="publish-studio-modal",t.className="publish-modal-overlay",t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),t.setAttribute("aria-hidden","true"),t.innerHTML=`
          ...
          <div class="publish-form-grid-2">
            <div class="publish-form-group">
              ...
              <select id="pub-category" class="publish-select">
                <option value="${d.INDIA_POLITICS}">🏛️ Indian Politics & Governance</option>
                <option value="${d.INDIA}">🇮🇳 India National News</option>
                <option value="${d.WORLD}">🌍 World & Global Affairs</option>
                <option value="${d.BUSINESS}">📈 Business, Economy & Markets</option>
                <option value="${d.TECH}">🤖 Technology & AI</option>
                <option value="${d.SCIENCE}">🚀 Science & Space</option>
    ...
    `,document.body.appendChild(t),this.modalEl=t}attachEventListeners(){if(!this.modalEl)return;const e=this.modalEl.querySelector("#btn-close-publish-modal"),t=this.modalEl.querySelector("#btn-cancel-publish");e&&e.addEventListener("click",()=>this.close()),t&&t.addEventListener("click",()=>this.close()),this.modalEl.addEventListener("click",o=>{o.target===this.modalEl&&this.close()}),window.addEventListener("keydown",o=>{o.key==="Escape"&&this.isOpen&&this.close()});const s=this.modalEl.querySelectorAll(".preset-photo-btn"),a=this.modalEl.querySelector("#pub-custom-image-url"),i=this.modalEl.querySelector("#pub-image-preview");s.forEach(o=>{o.addEventListener("click",()=>{s.forEach(h=>h.classList.remove("active")),o.classList.add("active");const l=o.getAttribute("data-url");this.selectedImageUrl=l,a&&(a.value=l),i&&(i.src=l)})}),a&&a.addEventListener("input",()=>{s.forEach(o=>o.classList.remove("active")),this.selectedImageUrl=a.value.trim(),i&&(i.src=this.selectedImageUrl)});const n=this.modalEl.querySelector("#publish-studio-form");n&&n.addEventListener("submit",o=>{o.preventDefault(),this.handleSubmit()})}open(e=null){this.modalEl||this.injectModalHtml();const t=f.getCurrentUser(),s=this.modalEl.querySelector("#pub-byline");s&&!e&&(t?s.value=t.displayName||t.email.split("@")[0]:s.value="What's Going On Editorial Desk"),this.isOpen=!0,this.modalEl.classList.add("active"),this.modalEl.setAttribute("aria-hidden","false"),document.body.style.overflow="hidden",e?(this.editingArticleId=e.id,this.modalEl.querySelector("#pub-headline").value=e.title||"",this.modalEl.querySelector("#pub-byline").value=e.source||"",this.modalEl.querySelector("#pub-category").value=e.category||d.INDIA_POLITICS,this.modalEl.querySelector("#pub-excerpt").value=e.snippet||"",this.modalEl.querySelector("#pub-content").value=e.content||"",this.modalEl.querySelector("#pub-custom-image-url").value=e.image||"",this.modalEl.querySelector("#pub-image-preview").src=e.image||"",e.keyPoints&&(this.modalEl.querySelector("#pub-keypoints").value=e.keyPoints.join(`
`))):this.editingArticleId=null}close(){this.modalEl&&(this.isOpen=!1,this.modalEl.classList.remove("active"),this.modalEl.setAttribute("aria-hidden","true"),document.body.style.overflow="")}handleSubmit(){const e=this.modalEl.querySelector("#pub-headline").value.trim(),t=this.modalEl.querySelector("#pub-byline").value.trim()||"What's Going On Editorial",s=this.modalEl.querySelector("#pub-dateline").value,a=this.modalEl.querySelector("#pub-category").value,i=this.modalEl.querySelector("#pub-readtime").value.trim()||"3 min read",n=this.selectedImageUrl||this.presetImages[0].url,o=this.modalEl.querySelector("#pub-excerpt").value.trim(),l=this.modalEl.querySelector("#pub-content").value.trim(),h=this.modalEl.querySelector("#pub-keypoints").value.trim(),c=h?h.split(`
`).map(u=>u.trim()).filter(Boolean):["Comprehensive on-the-ground intelligence and editorial assessment.","Analysis corroborated by verified primary sources and statutory records.","Implications reviewed by What's Going On editorial desk."],r={id:this.editingArticleId||"user-art-"+Date.now(),title:e,source:t,sourceId:"user-published",category:a,region:a===d.WORLD?"global":"india",isPolitics:a===d.INDIA_POLITICS||a===d.INDIA,isUserPublished:!0,link:"#user-story-"+Date.now(),image:n,pubDate:new Date().toISOString(),snippet:`${s} ${o}`,content:l,keyPoints:c,readTime:i};m.saveCustomArticle(r),this.close(),this.modalEl.querySelector("#publish-studio-form").reset(),this.onArticlePublishedCallback&&this.onArticlePublishedCallback(r)}}class se{constructor(e){this.onAuthSuccessCallback=e,this.modalEl=null,this.isOpen=!1,this.currentTab="signin"}init(){this.injectModalHtml(),this.attachEventListeners()}injectModalHtml(){const e=document.getElementById("auth-modal-overlay");e&&e.remove();const t=document.createElement("div");t.id="auth-modal-overlay",t.className="auth-modal-overlay",t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),t.setAttribute("aria-hidden","true"),t.innerHTML=`
    ...
    `}}class ne{constructor(){this.articles=[],this.currentCategory=d.ALL,this.searchQuery="",this.selectedSource="all",this.isLoading=!1,this.header=null,this.ticker=null,this.newsGrid=null,this.currentAffairs=null,this.modal=null,this.publishModal=null,this.authModal=null,this.adminPanel=null}async init(){this.applyTheme(m.getTheme()),this.initComponents(),this.renderCategoryNavigation(),await this.fetchNews(!1),this.initBackToTop()}applyTheme(e){document.documentElement.setAttribute("data-theme",e),m.setTheme(e);const t=document.getElementById("theme-icon-sun");t&&(e==="light"?t.innerHTML='<path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path>':t.innerHTML=`
      ...
      `;e.innerHTML=a+i,e.addEventListener("click",n=>{if(n.target.closest("#nav-btn-publish")){this.publishModal.open();return}const l=n.target.closest(".cat-tab");if(l){const h=l.getAttribute("data-category");this.setCategory(h)}})}t&&(t.innerHTML=`

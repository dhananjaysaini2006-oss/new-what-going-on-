...
Official authorities and reporting correspondents on the ground continue to monitor developments closely as further communiqués and statements are anticipated in the coming hours.`}resolveContextualNewsImage(e,t,a,s,i=0){if(e&&!e.includes("1x1")&&!e.includes("pixel")&&!e.includes("favicon"))return e;const n=`${t} ${a}`.toLowerCase();return s.region==="india"||s.isPolitics||n.includes("india")||n.includes("delhi")?n.includes("supreme court")||n.includes("high court")||n.includes("judge")||n.includes("judiciary")||n.includes("verdict")||n.includes("bench")||n.includes("bail")||n.includes("cbi")||n.includes("ed")?v.judiciary_law[0]:n.includes("isro")||n.includes("space")||n.includes("satellite")||n.includes("chandrayaan")||n.includes("drdo")||n.includes("missile")?v.defense_isro[i%v.defense_isro.length]:n.includes("rupee")||n.includes("rbi")||n.includes("reserve bank")||n.includes("bank")||n.includes("sensex")||n.includes("nifty")||n.includes("gdp")||n.includes("inflation")||n.includes("finance minister")?v.economy_rbi[i%v.economy_rbi.length]:n.includes("solar")||n.includes("renewable")||n.includes("power")||n.includes("green energy")?v.clean_energy[0]:n.includes("semiconductor")||n.includes("chip")||n.includes("fab")||n.includes("hardware")?v.technology[0]:v.parliament_governance[i%v.parliament_governance.length]:$[i%$.length]}isPoliticalArticle(e,t){const a=`${e} ${t}`.toLowerCase();return["parliament","lok sabha","rajya sabha","bjp","congress","election","elections","minister","cabinet","supreme court","chief minister","prime minister","modi","rahul gandhi","governor","mla","mp","assembly","ordinance","bill","judiciary","constitution","poll","party","yogi","kejriwal","shah","niti aayog","electoral","high court"].some(i=>a.includes(i))}parseDateSafe(e){if(!e)return new Date().toISOString();const t=new Date(e);return isNaN(t.getTime())?new Date().toISOString():t.toISOString()}stripHtml(e){if(!e)return"";const t=document.createElement("DIV");return t.innerHTML=e,t.textContent||t.innerText||""}extractCleanSnippet(e){const t=this.stripHtml(e);if(!t)return"Read the complete verified story on the original publisher broadcast.";const a=t.replace(/\s+/g," ").trim();return a.length>260?a.substring(0,257)+"...":a}extractImageFromXml(e,t){const a=Array.from(e.querySelectorAll("enclosure"));for(const n of a){const o=n.getAttribute("type")||"",r=n.getAttribute("url");if(r&&(o.startsWith("image")||r.match(/\.(jpeg|jpg|png|webp)/i)))return r}const s=Array.from(e.getElementsByTagNameNS("*","content")).concat(Array.from(e.getElementsByTagNameNS("*","thumbnail")));for(const n of s){const o=n.getAttribute("url");if(o&&!o.includes("1x1")&&!o.includes("analytics")&&!o.includes("logo"))return o}const i=e.querySelector("storyimage, thumb, image > url, fullimage");if(i&&i.textContent){const n=i.textContent.trim();if(n.startsWith("http"))return n}return this.extractImageFromHtml(t)}extractImageFromHtml(e){if(e){const t=e.matchAll(/<img[^>]+(?:src|data-src|data-original)=["']([^"']+)["']/gi);for(const a of t){const s=a[1];if(s&&!s.includes("feedburner")&&!s.includes("analytics")&&!s.includes("1x1")&&!s.includes("pixel")&&!s.includes("badge")&&!s.includes("button")&&!s.includes("icon"))return s}}return null}generateKeyPoints(e,t,a){return[`Primary Development: ${e}`,`Core Briefing: ${t.length>30?t.substring(0,140)+"...":"Verified wire bulletin from "+a.name+"."}`,`Verified Media Wire: Real-time update monitored on ${a.name} editorial network.`]}}const j=new G;class F{constructor(){this.synth=window.speechSynthesis,this.currentUtterance=null,this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.callbacks=[],this.voices=[],this.synth&&(this.loadVoices(),speechSynthesis.onvoiceschanged!==void 0&&(speechSynthesis.onvoiceschanged=()=>this.loadVoices()))}loadVoices(){this.voices=this.synth.getVoices()}getBestVoice(){return this.voices.length||this.loadVoices(),this.voices.find(t=>t.lang.startsWith("en")&&(t.name.includes("Natural")||t.name.includes("Google")||t.name.includes("Premium")))||this.voices.find(t=>t.lang.startsWith("en"))||this.voices[0]}speakArticle(e,t){if(!this.synth){console.warn("Speech synthesis not supported in this browser.");return}if(this.isPlaying&&this.currentArticleId===e.id){this.isPaused?this.resume():this.pause();return}this.stop();const a=e.title?e.title.replace(/<[^>]*>?/gm,""):"",s=e.snippet?e.snippet.replace(/<[^>]*>?/gm,""):"",i=e.content?e.content.replace(/<[^>]*>?/gm,""):"",n=`Story from ${e.source}. Headline: ${a}. Summary: ${s||i}`;this.currentUtterance=new SpeechSynthesisUtterance(n),this.currentUtterance.voice=this.getBestVoice(),this.currentUtterance.rate=1,this.currentUtterance.pitch=1,this.currentArticleId=e.id,this.isPlaying=!0,this.isPaused=!1,this.currentUtterance.onstart=()=>{this.notifyStateChange("playing",e)},this.currentUtterance.onend=()=>{this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.notifyStateChange("ended",e),t&&t()},this.currentUtterance.onerror=o=>{console.warn("Speech synthesis error",o),this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.notifyStateChange("ended",e)},this.synth.speak(this.currentUtterance)}pause(){this.synth&&this.isPlaying&&!this.isPaused&&(this.synth.pause(),this.isPaused=!0,this.notifyStateChange("paused"))}resume(){this.synth&&this.isPlaying&&this.isPaused&&(this.synth.resume(),this.isPaused=!1,this.notifyStateChange("playing"))}stop(){this.synth&&(this.synth.cancel(),this.isPlaying=!1,this.isPaused=!1,this.currentArticleId=null,this.notifyStateChange("stopped"))}subscribe(e){return this.callbacks.push(e),()=>{this.callbacks=this.callbacks.filter(t=>t!==e)}}notifyStateChange(e,t=null){this.callbacks.forEach(a=>a({state:e,article:t,articleId:this.currentArticleId,isPlaying:this.isPlaying,isPaused:this.isPaused}))}}const T=new F,V="modulepreload",K=function(p){return"/"+p},D={},L=function(e,t,a){let s=Promise.resolve();if(t&&t.length>0){let n=function(c){return Promise.all(c.map(d=>Promise.resolve(d).then(h=>({status:"fulfilled",value:h}),h=>({status:"rejected",reason:h}))))};document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),r=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));s=n(t.map(c=>{if(c=K(c),c in D)return;D[c]=!0;const d=c.endsWith(".css"),h=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${h}`))return;const m=document.createElement("link");if(m.rel=d?"stylesheet":V,d||(m.as="script"),m.crossOrigin="",m.href=c,r&&m.setAttribute("nonce",r),document.head.appendChild(m),d)return new Promise((g,k)=>{m.addEventListener("load",g),m.addEventListener("error",()=>k(new Error(`Unable to preload CSS for ${c}`)))})}))}function i(n){const o=new Event("vite:preloadError",{cancelable:!0});if(o.payload=n,window.dispatchEvent(o),!o.defaultPrevented)throw n}return s.then(n=>{for(const o of n||[])o.status==="rejected"&&i(o.reason);return e().catch(i)})};class J{constructor(e,t,a,s,i,n,o){this.onRefreshCallback=e,this.onSearchCallback=t,this.onThemeToggleCallback=a,this.onIntervalChangeCallback=s,this.onPublishClickCallback=i,this.onAuthClickCallback=n,this.onAdminClickCallback=o,this.remainingSeconds=u.getRefreshInterval(),this.totalSeconds=this.remainingSeconds,this.countdownTimer=null,this.clockTimer=null}init(){this.renderHeader(),this.startLiveClock(),this.startCountdown(),this.attachEventListeners(),this.updateBookmarkBadge(),this.initMobileDrawer()}renderHeader(){const e=document.getElementById("top-utility-bar"),t=document.getElementById("main-header");e&&(e.innerHTML=`
      ...
      `)}initMobileDrawer(){const e=document.getElementById("mobile-nav-drawer"),t=document.getElementById("mobile-drawer-overlay"),a=document.getElementById("btn-mobile-drawer-close"),s=document.getElementById("btn-hamburger"),i=document.getElementById("mobile-drawer-categories"),n=()=>{e&&e.classList.add("open"),t&&(t.style.display="block")},o=()=>{e&&e.classList.remove("open"),t&&(t.style.display="none")};s&&s.addEventListener("click",n),a&&a.addEventListener("click",o);const r=[{id:"all",label:"⚡ All News (Front Page)"},{id:"india-politics",label:"🏛️ Indian Politics"},{id:"india",label:"🇮🇳 India Wire"},{id:"world",label:"🌍 Global Wire"},{id:"business",label:"📊 Markets & Economy"},{id:"google-news",label:"🌐 Google News"},{id:"current-affairs",label:"⭐ Current Affairs Hub"},{id:"tech",label:"🤖 Tech & AI"},{id:"science",label:"🚀 Science & Defense"},{id:"saved",label:"🔖 Saved Stories"}];i&&(i.innerHTML=r.map(c=>`
        <button
        ...
        >${c.label}</button>
      `).join(""),i.querySelectorAll(".mobile-drawer-cat-btn").forEach(c=>{c.addEventListener("click",()=>{const d=document.querySelector(`.cat-tab[data-category="${c.dataset.cat}"]`);d&&d.click(),o(),window.scrollTo({top:0,behavior:"smooth"})})}))}startLiveClock(){const e=()=>{const t=new Date,a=document.getElementById("current-clock-time"),s=document.getElementById("current-clock-date");a&&(a.textContent=t.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0})),s&&(s.textContent=t.toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric",year:"numeric"}))};e(),this.clockTimer=setInterval(e,1e3)}startCountdown(){this.countdownTimer&&clearInterval(this.countdownTimer),this.totalSeconds=u.getRefreshInterval(),this.remainingSeconds=this.totalSeconds,this.nextRefreshTime=Date.now()+this.totalSeconds*1e3;const e=document.getElementById("countdown-circle-bar"),t=document.getElementById("countdown-text"),a=2*Math.PI*7;e&&(e.style.strokeDasharray=`${a}`);const s=()=>{const i=Date.now(),n=this.nextRefreshTime-i,o=Math.max(0,Math.ceil(n/1e3));this.remainingSeconds=o;const r=Math.floor(this.remainingSeconds/60),c=this.remainingSeconds%60;if(t&&(t.textContent=`${r.toString().padStart(2,"0")}:${c.toString().padStart(2,"0")}`),e){const d=Math.min(1,Math.max(0,this.remainingSeconds/this.totalSeconds)),h=a*(1-d);e.style.strokeDashoffset=`${h}`}o<=0&&(this.resetCountdown(),t&&(t.textContent="Syncing..."),this.onRefreshCallback&&this.onRefreshCallback(!0))};s(),this.countdownTimer=setInterval(s,1e3),this.visibilityListenerAttached||(this.visibilityListenerAttached=!0,document.addEventListener("visibilitychange",()=>{document.visibilityState==="visible"&&s()}))}resetCountdown(){this.totalSeconds=u.getRefreshInterval(),this.remainingSeconds=this.totalSeconds,this.nextRefreshTime=Date.now()+this.totalSeconds*1e3}updateLastSyncTime(e=new Date){const t=document.getElementById("last-sync-time");if(t){const a=e.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!0});t.textContent=`Synced: ${a}`}}updateBookmarkBadge(){const e=document.getElementById("header-bookmark-badge"),t=u.getBookmarks().length;e&&(e.textContent=t,e.style.display=t>0?"flex":"none")}updateUserAuthState(e){const t=document.getElementById("header-auth-container"),a=document.getElementById("topbar-auth-pill"),s=document.getElementById("btn-open-publish-modal"),i=e&&(e.role==="admin"||e.email&&e.email.toLowerCase()==="dhananjaysaini2006@gmail.com");if(s&&(s.style.display=i?"inline-flex":"none"),e){if(a&&(a.innerHTML=`
        ...
        `;const n=t.querySelector("#btn-header-admin-panel");n&&n.addEventListener("click",()=>{this.onAdminClickCallback&&this.onAdminClickCallback()});const o=t.querySelector("#btn-header-signout");o&&o.addEventListener("click",async()=>{const{firebaseService:r}=await L(async()=>{const{firebaseService:c}=await Promise.resolve().then(()=>te);return{firebaseService:c}},void 0);await r.signOut()})}}else{if(a){a.innerHTML=`
        ...
        `;const n=a.querySelector("#btn-topbar-signin");n&&n.addEventListener("click",r=>{r.preventDefault(),this.onAuthClickCallback&&this.onAuthClickCallback("signin")});const o=a.querySelector("#btn-topbar-signup");o&&o.addEventListener("click",r=>{r.preventDefault(),this.onAuthClickCallback&&this.onAuthClickCallback("signup")})}if(t){t.innerHTML=`
        ...
        `;const n=t.querySelector("#btn-header-signin");n&&n.addEventListener("click",()=>{this.onAuthClickCallback&&this.onAuthClickCallback("signin")});const o=t.querySelector("#btn-header-signup");o&&o.addEventListener("click",()=>{this.onAuthClickCallback&&this.onAuthClickCallback("signup")})}}}attachEventListeners(){const e=document.getElementById("btn-header-refresh");e&&e.addEventListener("click",()=>{e.classList.add("spinning"),this.resetCountdown(),this.onRefreshCallback&&this.onRefreshCallback(!1).finally(()=>{setTimeout(()=>e.classList.remove("spinning"),600)})});const t=document.getElementById("select-refresh-interval");t&&(t.value=u.getRefreshInterval().toString(),t.addEventListener("change",o=>{const r=parseInt(o.target.value,10);u.setRefreshInterval(r),this.startCountdown(),this.onIntervalChangeCallback&&this.onIntervalChangeCallback(r)}));const a=document.getElementById("global-search-input");if(a){let o=null;a.addEventListener("input",r=>{clearTimeout(o),o=setTimeout(()=>{this.onSearchCallback&&this.onSearchCallback(r.target.value.trim())},250)}),window.addEventListener("keydown",r=>{r.key==="/"&&document.activeElement!==a&&(r.preventDefault(),a.focus())})}const s=document.getElementById("btn-brand-home");s&&s.addEventListener("click",()=>{this.onSearchCallback&&this.onSearchCallback("");const o=document.querySelector('.cat-tab[data-category="all"]');o&&o.click()});const i=document.getElementById("btn-open-publish-modal");i&&i.addEventListener("click",()=>{this.onPublishClickCallback&&this.onPublishClickCallback()});const n=document.getElementById("btn-theme-toggle");n&&n.addEventListener("click",()=>{this.onThemeToggleCallback&&this.onThemeToggleCallback()})}}class Y{constructor(e){this.onArticleClickCallback=e,this.articles=[]}render(e){this.articles=e||[];const t=document.getElementById("breaking-news-ticker-container");if(!t)return;if(this.articles.length===0){t.style.display="none";return}t.style.display="block";const a=this.articles.slice(0,10),i=[...a,...a].map((o,r)=>`
    ...
    `,t.querySelectorAll(".ticker-item").forEach(o=>{o.addEventListener("click",()=>{const r=o.getAttribute("data-id"),c=this.articles.find(d=>d.id===r)||this.articles[0];this.onArticleClickCallback&&c&&this.onArticleClickCallback(c)})})}}class Q{constructor(e,t){this.onArticleClickCallback=e,this.onBookmarkToggleCallback=t,this.viewMode=u.getViewMode()}setViewMode(e){this.viewMode=e,u.setViewMode(e)}formatTimeAgo(e){try{const t=new Date(e),s=new Date-t,i=Math.floor(s/(1e3*60));if(i<1)return"Just now";if(i<60)return`${i}m ago`;const n=Math.floor(i/60);return n<24?`${n}h ago`:`${Math.floor(n/24)}d ago`}catch{return"Recently"}}render(e,t={}){const a=document.getElementById("main-news-feed");if(!a)return;const{category:s="all",searchQuery:i="",sourceFilter:n="all"}=t;if(!e||e.length===0){const k=t.category==="saved";a.innerHTML=k?`<div class="empty-state-premium">
          ...
          </div>`;return}if(s===l.ALL&&!i&&n==="all"){this.renderHomeAllNews(e,a);return}const o=!i&&e.length>0,r=o?e[0]:null,c=o?e.slice(1):e,d=r?this.buildHeroCardHtml(r):"",h=this.buildArticlesGridHtml(c),m=s===l.BUSINESS&&!i?this.buildMarketIndicesDashboardHtml():"";let g="Latest Stories";i?g=`Search Results: "${i}"`:s===l.INDIA_POLITICS?g="🏛️ Indian Politics & National Governance":s===l.INDIA?g="🇮🇳 Indian Media Wires":s===l.WORLD?g="🌍 Global News Wires & International":s===l.BUSINESS?g="📊 Global & Indian Business Wire":s===l.GOOGLE_NEWS?g="🌐 Google News Live Wire":s===l.SAVED?g="🔖 Saved Bookmarks":g=`${s.toUpperCase()} Wire`,a.innerHTML=`
      ${m}
    ...
    `}renderHomeAllNews(e,t){const a=e[0],s=e.slice(1,5),i=e.filter(h=>h.region==="india"||h.category===l.INDIA||h.category===l.INDIA_POLITICS),n=e.filter(h=>h.region==="global"||h.category===l.WORLD||h.category===l.GOOGLE_NEWS),o=e.filter(h=>h.category===l.BUSINESS),r=e.filter(h=>h.category===l.TECH||h.category===l.SCIENCE),c=e.slice(5,13),d=a.region==="india"?"NEW DELHI —":a.region==="global"?"GLOBAL WIRE —":"DISPATCH —";t.innerHTML=`
        ...
        <div class="hindu-lead-col" data-id="${a.id}">
          ...
          ${a.keyPoints&&a.keyPoints.length>0?`
            <div class="modal-key-points" style="margin: 4px 0 12px; padding: 14px 18px;">
              ...
              <ul style="margin: 0; padding-left: 18px; font-size: 0.9rem; color: var(--text-secondary); line-height: 1.55;">
                ${a.keyPoints.slice(0,2).map(h=>`<li>${h}</li>`).join("")}
              </ul>
            </div>
          `:""}
          ...
          <div class="hindu-lead-footer">
            ...
            <div style="display: flex; gap: 8px;">
              <button class="btn-card-action ${u.isBookmarked(a.id,a.link)?"bookmarked":""}" data-action="bookmark" title="Bookmark">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="${u.isBookmarked(a.id,a.link)?"currentColor":"none"}" stroke="currentColor" stroke-width="2">
                  <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path>
                </svg>
              </button>
            </div>
      ...
      `:""}
    `,this.attachCardEventListeners(e)}buildHeroCardHtml(e,t="LIVE LEAD STORY"){const a=u.isBookmarked(e.id,e.link),s=e.region==="india"||e.isPolitics;return`
    ...
    `}buildArticlesGridHtml(e){return this.viewMode==="grid"?`
        <div class="news-grid-cards">
          ${e.map(t=>{const a=u.isBookmarked(t.id,t.link);return`
      ...
      `}attachCardEventListeners(e){const t=document.getElementById("main-news-feed");t&&t.addEventListener("click",a=>{const s=a.target.closest("[data-action]"),i=a.target.closest("[data-id]");if(!i)return;const n=i.getAttribute("data-id"),o=e.find(r=>r.id===n);if(o)if(s){const r=s.getAttribute("data-action");if(r==="open-article")this.onArticleClickCallback&&this.onArticleClickCallback(o);else if(r==="bookmark"){const c=u.toggleBookmark(o);s.classList.toggle("bookmarked",c);const d=s.querySelector("svg");d&&d.setAttribute("fill",c?"currentColor":"none"),this.onBookmarkToggleCallback&&this.onBookmarkToggleCallback(o,c)}else r==="speak-article"?T.speakArticle(o):r==="share"&&(navigator.share?navigator.share({title:o.title,text:o.snippet,url:o.link||window.location.href}).catch(()=>{}):(navigator.clipboard.writeText(o.link||window.location.href),alert("Article link copied to clipboard!")))}else a.target.closest("button")||this.onArticleClickCallback&&this.onArticleClickCallback(o)})}}class X{constructor(){this.activeTopic="all",this.currentAffairsList=[..._]}render(){const e=document.getElementById("main-news-feed");if(!e)return;const t=this.activeTopic==="all"?this.currentAffairsList:this.currentAffairsList.filter(i=>i.category===this.activeTopic),s=new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"});e.innerHTML=`
...
• Why: ${o.why}
• Impact: ${o.impact}
• Stakeholders: ${o.who}`;navigator.clipboard.writeText(r),i.innerHTML='<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg> Copied!',setTimeout(()=>{i.innerHTML='<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> Copy Briefing'},2e3)}})}),document.querySelectorAll(".quiz-option-btn").forEach(i=>{i.addEventListener("click",()=>{const n=i.getAttribute("data-ca-id"),o=parseInt(i.getAttribute("data-opt-idx"),10),r=this.currentAffairsList.find(d=>d.id===n),c=document.getElementById(`quiz-ans-${n}`);r&&c&&(c.classList.add("show"),o===r.quiz.correct?(i.style.borderColor="#10b981",i.style.background="rgba(16, 185, 129, 0.2)",i.style.color="#34d399"):(i.style.borderColor="#ef4444",i.style.background="rgba(239, 68, 68, 0.2)",i.style.color="#f87171"))})})}}class Z{constructor(e){this.onBookmarkToggleCallback=e,this.currentArticle=null,this.fontSize=17,this.init()}init(){this.renderModalContainer(),this.attachGlobalListeners()}renderModalContainer(){let e=document.getElementById("article-reader-modal");e||(e=document.createElement("div"),e.id="article-reader-modal",e.className="modal-overlay",document.body.appendChild(e))}open(e){this.currentArticle=e,u.markAsRead(e.id);const t=document.getElementById("article-reader-modal");if(!t)return;const a=u.isBookmarked(e.id,e.link),i=(e.content||e.snippet||"").split(`
...
`).filter(r=>r.trim().length>0);t.innerHTML=`
    ...
    `,t.classList.add("active"),document.body.style.overflow="hidden",this.attachModalEvents(e);const n=t.querySelector(".modal-scroll-content"),o=document.getElementById("modal-read-progress-bar");n&&o&&n.addEventListener("scroll",()=>{const r=n.scrollTop,c=n.scrollHeight-n.clientHeight,d=c>0?Math.min(100,r/c*100):0;o.style.width=d+"%"},{passive:!0})}close(){const e=document.getElementById("article-reader-modal");e&&(e.classList.remove("active"),document.body.style.overflow="")}attachModalEvents(e){const t=document.getElementById("modal-close-btn");t&&t.addEventListener("click",()=>this.close());const a=document.getElementById("modal-article-body-text"),s=document.getElementById("modal-font-inc"),i=document.getElementById("modal-font-dec");s&&a&&s.addEventListener("click",()=>{this.fontSize<24&&(this.fontSize+=1.5,a.style.fontSize=`${this.fontSize}px`)}),i&&a&&i.addEventListener("click",()=>{this.fontSize>14&&(this.fontSize-=1.5,a.style.fontSize=`${this.fontSize}px`)});const n=document.getElementById("modal-audio-btn");n&&n.addEventListener("click",()=>{T.speakArticle(e)});const o=document.getElementById("modal-bookmark-btn");o&&o.addEventListener("click",()=>{const h=u.toggleBookmark(e);o.classList.toggle("bookmarked",h);const m=o.querySelector("svg");m&&m.setAttribute("fill",h?"currentColor":"none"),this.onBookmarkToggleCallback&&this.onBookmarkToggleCallback(e,h)});const r=document.getElementById("modal-copy-link-btn");r&&r.addEventListener("click",()=>{navigator.clipboard.writeText(e.link||window.location.href),r.textContent="Copied!",setTimeout(()=>{r.textContent="Copy Link"},2e3)});const c=document.getElementById("modal-whatsapp-btn");c&&c.addEventListener("click",async()=>{const h={title:e.title,text:e.snippet||e.title,url:e.link||window.location.href};try{if(navigator.share)await navigator.share(h);else{await navigator.clipboard.writeText(e.link||window.location.href);const m=c.innerHTML;c.textContent="Link Copied!",setTimeout(()=>{c.innerHTML=m},2e3)}}catch{}});const d=document.getElementById("modal-delete-user-article-btn");d&&d.addEventListener("click",()=>{confirm("Are you sure you want to unpublish this story from What's Going On?")&&(u.deleteCustomArticle(e.id),this.close(),window.location.reload())})}attachGlobalListeners(){window.addEventListener("keydown",t=>{t.key==="Escape"&&this.close()});const e=document.getElementById("article-reader-modal");e&&e.addEventListener("click",t=>{t.target===e&&this.close()})}}const E={apiKey:"AIzaSyDemoPlaceholderKeyForWhatsGoingOn",authDomain:"whats-going-on-news.firebaseapp.com",projectId:"whats-going-on-news",storageBucket:"whats-going-on-news.appspot.com",messagingSenderId:"123456789012",appId:"1:123456789012:web:abcdef123456"},y={CURRENT_USER:"wgo_auth_user_v1",REGISTERED_USERS:"wgo_registered_users_v1",BREAKING_ALERT:"wgo_breaking_alert_v1"},R=1440*60*1e3;function N(p){try{return btoa(encodeURIComponent(p+"_wgo_salt_2026"))}catch{return p}}function P(p,e){return N(p)===e}const w={e:"dhananjaysaini2006@gmail.com",h:N("242622")};class ee{constructor(){this.currentUser=null,this.authListeners=[],this.isFirebaseLive=!1,this.firebaseApp=null,this.firebaseAuth=null,this.init()}async init(){this.loadSavedSession(),this.ensureAdminExists();try{if(E.apiKey&&!E.apiKey.includes("Placeholder")){const{initializeApp:e}=await L(async()=>{const{initializeApp:a}=await import("./index.esm-D_m3LDGF.js");return{initializeApp:a}},__vite__mapDeps([0,1])),{getAuth:t}=await L(async()=>{const{getAuth:a}=await import("./index.esm-8z07djW7.js");return{getAuth:a}},__vite__mapDeps([2,1]));this.firebaseApp=e(E),this.firebaseAuth=t(this.firebaseApp),this.isFirebaseLive=!0,console.info("Connected to live Firebase Cloud project.")}}catch{console.info("Using local Firebase Authentication adapter.")}}loadSavedSession(){try{const e=localStorage.getItem(y.CURRENT_USER);e&&(this.currentUser=JSON.parse(e))}catch{this.currentUser=null}}ensureAdminExists(){const e=this.getAllUsers();if(!e.some(a=>a.email.toLowerCase()===w.e.toLowerCase())){e.unshift({uid:"admin-ds-wgo",email:w.e,displayName:"Dhananjay Saini",role:"admin",passwordHash:w.h,createdAt:"2026-09-02T00:00:00.000Z"});try{localStorage.setItem(y.REGISTERED_USERS,JSON.stringify(e))}catch{}}}getAllUsers(){try{const e=localStorage.getItem(y.REGISTERED_USERS);return e?JSON.parse(e):[]}catch{return[]}}getCurrentUser(){return this.currentUser}isAdmin(){return this.currentUser&&(this.currentUser.role==="admin"||this.currentUser.email&&this.currentUser.email.toLowerCase()===w.e.toLowerCase())}onAuthStateChanged(e){return this.authListeners.push(e),e(this.currentUser),()=>{this.authListeners=this.authListeners.filter(t=>t!==e)}}notifyAuthChanged(){this.authListeners.forEach(e=>{try{e(this.currentUser)}catch(t){console.error("Error in auth listener:",t)}})}async signIn(e,t){const a=(e||"").trim().toLowerCase(),s=(t||"").trim();if(!a||!s)throw new Error("Please provide both email and password.");if(!a.includes("@"))throw new Error("Please enter a valid email address.");if(a===w.e.toLowerCase()){if(P(s,w.h))return this.currentUser={uid:"admin-ds-wgo",email:w.e,displayName:"Dhananjay Saini",role:"admin",photoURL:null},localStorage.setItem(y.CURRENT_USER,JSON.stringify(this.currentUser)),this.notifyAuthChanged(),this.currentUser;throw new Error("Incorrect password. Please try again.")}const n=this.getAllUsers().find(r=>r.email.toLowerCase()===a);if(!n)throw new Error("No account found with this email. Please sign up first.");if(!(n.passwordHash?P(s,n.passwordHash):n.passwordHash===s))throw new Error("Invalid password. Please try again.");return this.currentUser={uid:n.uid,email:n.email,displayName:n.displayName,role:n.role||"reader",photoURL:n.photoURL||null},localStorage.setItem(y.CURRENT_USER,JSON.stringify(this.currentUser)),this.notifyAuthChanged(),this.currentUser}async signUp(e,t,a=""){const s=(e||"").trim().toLowerCase(),i=(t||"").trim(),n=(a||"").trim()||s.split("@")[0];if(!s||!i)throw new Error("Please provide email and password.");if(!s.includes("@"))throw new Error("Please enter a valid email address.");if(i.length<8)throw new Error("Password must be at least 8 characters long.");const o=this.getAllUsers();if(o.some(c=>c.email.toLowerCase()===s))throw new Error("An account with this email already exists. Please sign in.");const r={uid:"user-"+Date.now(),email:s,displayName:n,role:"reader",passwordHash:N(i),createdAt:new Date().toISOString()};o.push(r);try{localStorage.setItem(y.REGISTERED_USERS,JSON.stringify(o))}catch(c){console.warn("Failed to save user",c)}return this.currentUser={uid:r.uid,email:r.email,displayName:r.displayName,role:r.role,photoURL:null},localStorage.setItem(y.CURRENT_USER,JSON.stringify(this.currentUser)),this.notifyAuthChanged(),this.currentUser}async signOut(){this.currentUser=null,localStorage.removeItem(y.CURRENT_USER),this.notifyAuthChanged()}getBreakingAlert(){try{const e=localStorage.getItem(y.BREAKING_ALERT);if(!e)return null;const t=JSON.parse(e);return t.timestamp&&Date.now()-new Date(t.timestamp).getTime()>R?(localStorage.removeItem(y.BREAKING_ALERT),null):t}catch{return null}}setBreakingAlert(e,t="high"){if(!this.isAdmin())throw new Error("Unauthorized. Only Admins can broadcast alerts.");const a={id:"alert-"+Date.now(),text:e,urgency:t,timestamp:new Date().toISOString(),author:this.currentUser.displayName,expiresAt:new Date(Date.now()+R).toISOString()};return localStorage.setItem(y.BREAKING_ALERT,JSON.stringify(a)),a}clearBreakingAlert(){if(!this.isAdmin())throw new Error("Unauthorized.");localStorage.removeItem(y.BREAKING_ALERT)}}const b=new ee,te=Object.freeze(Object.defineProperty({__proto__:null,FIREBASE_CONFIG:E,firebaseService:b},Symbol.toStringTag,{value:"Module"}));class ie{constructor(e){this.onArticlePublishedCallback=e,this.modalEl=null,this.isOpen=!1,this.editingArticleId=null,this.presetImages=[{label:"🏛️ Parliament & Central Vista",url:"https://images.unsplash.com/photo-1587474260584-136574528ed5?w=1200&auto=format&fit=crop&q=80"},{label:"⚖️ Supreme Court & Justice",url:"https://images.unsplash.com/photo-1589829545856-d10d557cf95f?w=1200&auto=format&fit=crop&q=80"},{label:"🪙 Indian Rupee & RBI",url:"https://images.unsplash.com/photo-1607604276583-eef5d076aa5f?w=1200&auto=format&fit=crop&q=80"},{label:"🚀 ISRO Space Rocket",url:"https://images.unsplash.com/photo-1517976487508-59a5a0da23b2?w=1200&auto=format&fit=crop&q=80"},{label:"🤖 Semiconductors & AI",url:"https://images.unsplash.com/photo-1518770660439-4636190af475?w=1200&auto=format&fit=crop&q=80"},{label:"🌍 Global Summit & Diplomacy",url:"https://images.unsplash.com/photo-1511578314322-379afb476865?w=1200&auto=format&fit=crop&q=80"}],this.selectedImageUrl=this.presetImages[0].url}init(){this.injectModalHtml(),this.attachEventListeners()}injectModalHtml(){const e=document.getElementById("publish-studio-modal");e&&e.remove();const t=document.createElement("div");t.id="publish-studio-modal",t.className="publish-modal-overlay",t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),t.setAttribute("aria-hidden","true"),t.innerHTML=`
    ...
    `,document.body.appendChild(t),this.modalEl=t}attachEventListeners(){if(!this.modalEl)return;const e=this.modalEl.querySelector("#btn-close-publish-modal"),t=this.modalEl.querySelector("#btn-cancel-publish");e&&e.addEventListener("click",()=>this.close()),t&&t.addEventListener("click",()=>this.close()),this.modalEl.addEventListener("click",o=>{o.target===this.modalEl&&this.close()}),window.addEventListener("keydown",o=>{o.key==="Escape"&&this.isOpen&&this.close()});const a=this.modalEl.querySelectorAll(".preset-photo-btn"),s=this.modalEl.querySelector("#pub-custom-image-url"),i=this.modalEl.querySelector("#pub-image-preview");a.forEach(o=>{o.addEventListener("click",()=>{a.forEach(c=>c.classList.remove("active")),o.classList.add("active");const r=o.getAttribute("data-url");this.selectedImageUrl=r,s&&(s.value=r),i&&(i.src=r)})}),s&&s.addEventListener("input",()=>{a.forEach(o=>o.classList.remove("active")),this.selectedImageUrl=s.value.trim(),i&&(i.src=this.selectedImageUrl)});const n=this.modalEl.querySelector("#publish-studio-form");n&&n.addEventListener("submit",o=>{o.preventDefault(),this.handleSubmit()})}open(e=null){if(!b.isAdmin()){alert("Access Restricted: Only the Admin (dhananjaysaini2006@gmail.com) has permission to write and publish news stories.");return}this.modalEl||this.injectModalHtml(),this.isOpen=!0,this.modalEl.classList.add("active"),this.modalEl.setAttribute("aria-hidden","false"),document.body.style.overflow="hidden",e?(this.editingArticleId=e.id,this.modalEl.querySelector("#pub-headline").value=e.title||"",this.modalEl.querySelector("#pub-byline").value=e.source||"",this.modalEl.querySelector("#pub-category").value=e.category||l.INDIA_POLITICS,this.modalEl.querySelector("#pub-excerpt").value=e.snippet||"",this.modalEl.querySelector("#pub-content").value=e.content||"",this.modalEl.querySelector("#pub-custom-image-url").value=e.image||"",this.modalEl.querySelector("#pub-image-preview").src=e.image||"",e.keyPoints&&(this.modalEl.querySelector("#pub-keypoints").value=e.keyPoints.join(`
`))):this.editingArticleId=null}close(){this.modalEl&&(this.isOpen=!1,this.modalEl.classList.remove("active"),this.modalEl.setAttribute("aria-hidden","true"),document.body.style.overflow="")}handleSubmit(){const e=this.modalEl.querySelector("#pub-headline").value.trim(),t=this.modalEl.querySelector("#pub-byline").value.trim()||"What's Going On Editorial",a=this.modalEl.querySelector("#pub-dateline").value,s=this.modalEl.querySelector("#pub-category").value,i=this.modalEl.querySelector("#pub-readtime").value.trim()||"3 min read",n=this.selectedImageUrl||this.presetImages[0].url,o=this.modalEl.querySelector("#pub-excerpt").value.trim(),r=this.modalEl.querySelector("#pub-content").value.trim(),c=this.modalEl.querySelector("#pub-keypoints").value.trim(),d=c?c.split(`
`).map(m=>m.trim()).filter(Boolean):["Comprehensive on-the-ground intelligence and editorial assessment.","Analysis corroborated by verified primary sources and statutory records.","Implications reviewed by What's Going On editorial desk."],h={id:this.editingArticleId||"user-art-"+Date.now(),title:e,source:t,sourceId:"user-published",category:s,region:s===l.WORLD?"global":"india",isPolitics:s===l.INDIA_POLITICS||s===l.INDIA,isUserPublished:!0,link:"#user-story-"+Date.now(),image:n,pubDate:new Date().toISOString(),snippet:`${a} ${o}`,content:r,keyPoints:d,readTime:i};u.saveCustomArticle(h),this.close(),this.modalEl.querySelector("#publish-studio-form").reset(),this.onArticlePublishedCallback&&this.onArticlePublishedCallback(h)}}class ae{constructor(e){this.onAuthSuccessCallback=e,this.modalEl=null,this.isOpen=!1,this.currentTab="signin"}init(){this.injectModalHtml(),this.attachEventListeners()}injectModalHtml(){const e=document.getElementById("auth-modal-overlay");e&&e.remove();const t=document.createElement("div");t.id="auth-modal-overlay",t.className="auth-modal-overlay",t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),t.setAttribute("aria-hidden","true"),t.innerHTML=`
    ...
    `;const s=e.querySelector("#admin-search-articles");s&&s.addEventListener("input",i=>{const n=i.target.value.toLowerCase().trim();e.querySelectorAll("#admin-articles-tbody tr").forEach(r=>{const c=r.textContent.toLowerCase();r.style.display=c.includes(n)?"":"none"})}),e.querySelectorAll(".btn-admin-delete").forEach(i=>{i.addEventListener("click",n=>{const o=n.target.getAttribute("data-del-id");confirm("Admin Action: Unpublish this story from What's Going On?")&&(u.deleteCustomArticle(o),this.switchTab("articles"),this.onArticleUpdatedCallback&&this.onArticleUpdatedCallback())})})}renderAlertsTab(e){const t=b.getBreakingAlert();e.innerHTML=`
    ...
    `;const a=e.querySelector("#admin-broadcast-form");a&&a.addEventListener("submit",i=>{i.preventDefault();const n=e.querySelector("#admin-alert-text").value.trim(),o=e.querySelector("#admin-alert-urgency").value,r=b.setBreakingAlert(n,o);alert("Breaking Alert has been broadcast live to all readers!"),this.switchTab("alerts"),this.onBreakingAlertCallback&&this.onBreakingAlertCallback(r)});const s=e.querySelector("#btn-clear-alert");s&&s.addEventListener("click",()=>{b.clearBreakingAlert(),alert("Active breaking alert has been revoked."),this.switchTab("alerts"),this.onBreakingAlertCallback&&this.onBreakingAlertCallback(null)})}renderFeedsTab(e){e.innerHTML=`
    ...
    `}renderUsersTab(e){const t=b.getAllUsers(),a=u.getCustomArticles(),s=u.getBookmarks();e.innerHTML=`
